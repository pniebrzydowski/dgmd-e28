...
const GameSettings = ({ players, setPlayers, onClear }) => {
  ...
  const remainingCharacters = characters.filter(
    (option) => players.findIndex((p) => p.name === option.name) === -1
  );
  ...
  const resetPlayers = () => {
    if (
      window.confirm(
        "Are you sure you want to clear the players? This will also clear the scoresheet and reset any ongoing game!"
      )
    ) {
      setPlayers([]);
  ...
  const characterSelectList = !houseFilter
    ...
    : remainingCharacters.filter(
        (character) => character.house === houseFilter
      );
          ...
          <ul>
            {players.map((player, idx) => (
              <li key={player.name}>
                ...
                {idx === 0 ? " (You)" : ""}
                <br />
                <span className="character-house">{player.house}</span>
              </li>
            ))}
          </ul>
          <button
            type="button"
            className="button-clearPlayers"
